    ...
    public class ProjectService : IProjectService
    {
        ...
        public async Task<ServiceResponse<UpdateProjectViewModel>> UpdateProject(int id, UpdateProjectViewModel model, CancellationToken cancellationToken = default)
        {
            ...
            if (selectedEmployeeIdToRemove.Count() > 0)
            {
                ...
                var isRemoveEmployees = await _unitOfWork.SaveChangeAsync() > 0;

                if (!isRemoveEmployees)
                {
                    response = ServiceResponse<UpdateProjectViewModel>.ErrorResult("Cannot save when delete employee in project !");
                    return response;
                }
            }
            ...
            if (newEmployeeId.Count() != 0)
            {
                ...
                var isAddProjectEmployee = await _unitOfWork.SaveChangeAsync() > 0;
                if (!isAddProjectEmployee)
                {
                    response = ServiceResponse<UpdateProjectViewModel>.ErrorResult("Cannot update employee into project");
                    return response;
                }
            }
            ...
            while (!isSuccess)
            {
                try
                {
                    isSuccess = await _unitOfWork.SaveChangeAsync(cancellationToken) > 0;
                }
                catch (DbUpdateConcurrencyException ex)
                {
